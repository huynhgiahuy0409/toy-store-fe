...
function () {
  ...
  ShoppingCartComponent.prototype.updateTotalPrice = function (orders) {
    this.pendingOrders = orders.map(function (item) {
      return __assign(__assign({}, item), {
        totalPrice: Math.round((100 - item.discountPercent) * item.priceUnit * item.quantity / 100 / 1000) * 1000
      });
    });
  };
  ...
  ShoppingCartComponent.prototype.changeQuantity = function (pendingOrderItem, value) {
    ...
    var foundOrder = pendingOrders.find(function (order) {
      return order.id === pendingOrderItem.id;
    });
  ...
  ShoppingCartComponent.prototype.removePendingOrder = function (id) {
    ...
    pendingOrders = pendingOrders.filter(function (item) {
      return item.id !== id;
    });
    console.log(pendingOrders);
    localStorage.setItem('pendingOrders', JSON.stringify(pendingOrders));
  ...
  ShoppingCartComponent = __decorate([core_1.Component({
