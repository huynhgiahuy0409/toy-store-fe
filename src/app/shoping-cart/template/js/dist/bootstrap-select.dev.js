...
(function (root, factory) {
  ...
  (function ($) {
    ...
    Selectpicker.VERSION = '1.12.2'; // part of this is duplicated in i18n/defaults-en_US.js. Make sure to update both.
    ...
    Selectpicker.prototype = {
      ...
      createDropdown: function createDropdown() {
        ...
        var donebutton = this.multiple && this.options.doneButton ? '<div class="bs-donebutton">' + '<div class="btn-group btn-block">' + '<button type="button" class="btn btn-sm btn-default btn-light">' + this.options.doneButtonText + '</button>' + '</div>' + '</div>' : '';
        var drop = '<div class="btn-group bootstrap-select' + showTick + inputGroup + '">' + '<button type="button" class="' + this.options.styleBase + ' dropdown-toggle" data-toggle="dropdown"' + autofocus + ' role="button">' + '<span class="filter-option pull-left"></span>&nbsp;' + '<span class="bs-caret">' + this.options.template.caret + '</span>' + '</button>' + '<div class="dropdown-menu open" role="combobox">' + header + searchbox + actionsbox + '<div class="dropdown-menu inner" role="listbox" aria-expanded="false">' + '</div>' + donebutton + '</div>' + '</div>';
        return $(drop);
      },
      ...
      createLi: function createLi() {
        ...
        var generateA = function generateA(content, index, classes, optgroup) {
          classes = 'dropdown-item ' + (classes || '');
          return '<a tabindex="0"' + (typeof classes !== 'undefined' && '' !== classes ? ' class="' + classes + '"' : '') + (typeof index !== 'undefined' && null !== index ? ' data-original-index="' + index + '"' : '') + (typeof optgroup !== 'undefined' && null !== optgroup ? 'data-optgroup="' + optgroup + '"' : '') + '>' + content + '</a>';
        };

        var generateDiv = function generateDiv(content, classes, optgroup) {
          return '<div' + (typeof classes !== 'undefined' && '' !== classes ? ' class="' + classes + '"' : '') + (typeof optgroup !== 'undefined' && null !== optgroup ? ' data-optgroup="' + optgroup + '"' : '') + '>' + content + '</div>';
        };
        /**
         * @param text
         * @param [classes]
        ...
        var generateSpan = function generateSpan(text, classes, inline, tokens) {
      ...
      render: function render(updateLi) {
        var that = this,
            notDisabled,
            $selectOptions = this.$element.find('option'); //Update the LI to match the SELECT
          ...
          if (that.$searchbox.val()) {
            ...
            if ($hideItems.length === $searchBase.length) {
              ...
              var $lisVisible = that.$lis.not('.d-none'),
                  $foundDiv; // hide divider if first or last visible, or if followed by another divider
