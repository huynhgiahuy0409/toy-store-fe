...
export class ShoppingCartService {
  ...
  summaryCartBSub: BehaviorSubject<SummaryCart> = new BehaviorSubject(
    this.initialSummaryCart
  );
  ...
  ): Observable<PendingOrderItem[]> | null {
    const userId = this.cookieService.get('uid');
    if (userId) {
      const url = `${DOMAIN}/api/cart/merge?userId=${userId}`;
